...
public:
    vector<int> topKFrequent(vector<int>& nums, int k) {
        ...
        vector<int> result;
        for(int i = buckets.size() - 1;  result.size() < k; i-- ){  // start from the highest possible frequency
            for(const int &num: buckets[i]){    // while inside the bucket[i] we add the number(s) in that bucket, i.e, if we're inside the bucket that contains all the numbers that 
                result.push_back(num);          // occur 3 times then we add one number to result while result.size() < k. Empty buckets are automatically skipped.
            }
        }
